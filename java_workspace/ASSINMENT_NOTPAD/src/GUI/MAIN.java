...
public class MAIN extends JFrame implements ActionListener/* MouseListener*/{
		...
		JScrollPane scroller = new JScrollPane(textfield);//it has the textfield(JTextArea) in it to select what its the container fork
		...
		private JMenuBar newMenuBar(){
			...
			String [] titles = {"FILE" , "EDIT","FORMAT","VIEW","HELP"};

			String [][] elements = {
				{"new1","new_window","open","save","save_as","page_setup","print","exit"},
				{"undo","cut","copy","paste","delete","search_with","find","find_next","find_previous","replace","goTo","select_All","Time_Date"},
				{"word_wrap","font"},
				{"Zoom","zoom_in","zoom_out","restore_default_zoom"},
				{"Status_bar","view_help","send_feedback","About_notepad"}
			};
		...
		private JMenu newMenu(String title, String[] elements) {
			...
			for(String element : elements) { //for each loop
				...
				menuitem.addActionListener(this);//implementing action listener class to jmenuitem
			}
			return menu;
		}
		...
		//create a constructor of the frame
		 MAIN(){
			...
			setResizable(true);
			setJMenuBar(newMenuBar());//add the menubar to the frame
			setTitle(" Note pad");
...
//			textfield.setBackground(new Color(133,60,60))
			...
			textfield.setFont(new Font("Lucida Calligraphy",Font.BOLD,10));
//			setFont(new Font("Swis721 BlkOul BT",Font.BOLD,20));
		...
		public void actionPerformed(ActionEvent onclick_event) {
			if(onclick_event.getSource().equals("copy")) {
				textfield.copy();
			}
			if(onclick_event.getSource().equals("cut") ) {
				textfield.cut();
			}
